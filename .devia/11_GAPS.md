...
|---|---|---|---|---|
| G1 | Should `devia check` grow ecosystem-specific gates (Python, Go, Rust) or stay deliberately generic? | Generic checks under-serve non-Node projects; specific ones multiply maintenance | Generic checks only; anything undetectable returns `SKIP` with the reason | open |
